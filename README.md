...
<p align="center">
  <img src="docs/images/icon.png" alt="Lucky Doctor Icon" width="120">
</p>
...
- Skill 用 `create_sticker.py` 从数据包 ZIP 生成：`PNG` 高分辨率二维码 + `A4` 可裁剪贴纸页（默认一页 6 枚、每枚 **70×50mm**；贴纸上为药名 + 自动居中二维码 + **底部记录 id**，枚更大/更多时自动分页），打印裁剪后贴到药盒。盒面较小需减少遮挡时用 `--sticker-size 宽x高` 调小；二维码边长也可用 `--qr-size-mm` 强制指定。
